...
export default async function CardTitle({ Organzation }: { Organzation: GProject }) {
  const participantsCount = Organzation.participants?.length || 0
  const analystsCount = Organzation.analysts?.length || 0
      ...
      <Card variant="outlined" className="rounded-3xl bg-white duration-300">
        <div className="flex flex-col items-center space-y-4 p-6 md:flex-row md:space-x-6 md:space-y-0">
          ...
          <div className="flex flex-col text-center md:text-left">
            {/* プロジェクト名 */}
            <h2 className="text-2xl font-bold text-gray-800">{Organzation.project_name}</h2>

            {/* プロジェクトの説明 */}
            <p className="mt-2 text-sm text-gray-600 md:text-base">{Organzation.project_description}</p>

            {/* プロジェクトコード */}
            <p className="mt-1 text-xs text-gray-500 md:text-sm">
              Project Code: <span className="font-medium text-gray-800">{Organzation.project_code}</span>
            </p>
